...
class ATM {
    ...
    public void withdrawCash(Person p, int entered_PIN, int amount){
        if(p.PIN == entered_PIN){
            if(p.balance < amount){
                System.out.println(amount + " is not available in your account, current balance is "+p.balance);
            } else {
                p.balance=p.balance-amount;
                System.out.println("Amount withdrawn "+amount +" updated balance is "+p.balance);
            }
        } else {
...
public class Banking {
    public static void main(String[] args) {
        ...
        Person p2=new Person("B", 2, 1900, 1233);
